...
def ccf(
    ...
    """Calculates the sample cross correlation function (CCF) across time lags between a "predictor" x_series and a "predicted" y_series.

    We calculate sample CCF as the set of sample correlations between and xt and yt+h (that is, x "lags" y)
    see https://stackoverflow.com/a/20463466/4696032
    """

    def time_pearson_corr(h: int) -> float:
        """Calculates time-dependent cross-correlations between two time series using Pearson Correlation Coefficient.
        ...
        ccf_lag = (
            np.sum(
                (partial_x - np.mean(partial_x))
                * (partial_y - np.mean(partial_y))
            )
            / c0
        )
        return round(ccf_lag, 3)
    ...
    def moving_block_bootstrap_ci(h: int) -> List[float]:
        ...
        # if array is not divisible by window size, we ceil blocks number and adjust the bootstrap array
        ...
        for i in range(n_boot):
            ...
            random_block_idx = random.choices(
                np.arange(len(x_kunsch_blocks)), k=block_number
            )

            # This allows to pick random blocks in both time series while preserving the time structure
            ...
            c0 = (
                float(n - h)
                * np.std(resampled_xarray)
                * np.std(resampled_yarray)
            )
            bootstrap_corr[i] = (
                np.sum(
